...
function SearchOptimizer({ items, onResultsChange }) {
  ...
  useEffect(() => {
    onResultsChange(sortedItems);
  }, [sortedItems, onResultsChange]);
...
export const generateSearchOptimizedContent = (
  content,
  location = "Czeladź"
) => {
  ...
  const optimizedContent = locationKeywords.reduce((acc, loc) => {
    const localContent = serviceKeywords
      .map(
        (service) =>
          `${service} ${loc}, ${service} w ${loc}, ${service} na ${
            loc === "Śląsk" ? "Śląsku" : loc
          }`
      )
      .join(", ");
  ...
  const mainLocationContent = serviceKeywords
    .map(
      (service) =>
...
export const enhanceProjectMetadata = (project) => {
  ...
  return {
    ...
    seoDescription: generateSearchOptimizedContent(
      project.description,
      project.location
    ),
    breadcrumbs: [
      ...
      { name: "Realizacje", url: "/realizacje" },
      {
        name:
          ...
          project.category,
        url: `/realizacje/${project.category}`,
      },
